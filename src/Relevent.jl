...
- decay-weighted interaction-history statistics that plug directly into
  ...
  into this package's ordinal full-risk-set estimator — computed via
...
using Networks: CoefficientTable, newton_fit, z_pvalues
...
export coef, coefnames, stderror, vcov, confint, loglikelihood, nobs, dof,
       aic, aicc, bic, coeftable
...
mutable struct _DecayAccum
    ...
    dyad_order::Dict{Tuple{Int,Int},Int}                # last event index
...
# R catalogue statistics use cumulative, unit-event counts. They deliberately
# do not inherit REM's decay/window/weight semantics; see the catalogue guide.
struct _CatalogueStatistic{K} <: AbstractStatistic
    n_actors::Int
    cache::_AccumCache
    function _CatalogueStatistic{K}(n_actors::Int) where K
        n_actors >= 2 || throw(ArgumentError("$K requires at least two actors"))
        new{K}(n_actors, _AccumCache())
    end
end

"""
    NIDSnd(n_actors) <: AbstractStatistic

Sender cumulative indegree divided by the number of past events. Before the first event, the value is 1/(n_actors-1), matching R relevent.
Cumulative unit-event semantics agree with `relevent::rem.dyad` on the
checked-in catalogue fixture. Supports both Relevent and REM compute interfaces.

# Example
```julia
using Relevent, REM
stat = NIDSnd(4)
compute(stat, InteractionHistory(), 1, 2, 0.0)
```
"""
const NIDSnd = _CatalogueStatistic{:NIDSnd}
export NIDSnd

"""
    NIDRec(n_actors) <: AbstractStatistic

Receiver cumulative indegree divided by the number of past events. Before the first event, the value is 1/(n_actors-1), matching R relevent.
Cumulative unit-event semantics agree with `relevent::rem.dyad` on the
checked-in catalogue fixture. Supports both Relevent and REM compute interfaces.
...
```
"""
const NIDRec = _CatalogueStatistic{:NIDRec}
export NIDRec

"""
    NODSnd(n_actors) <: AbstractStatistic

Sender cumulative outdegree divided by the number of past events. Before the first event, the value is 1/(n_actors-1), matching R relevent.
Cumulative unit-event semantics agree with `relevent::rem.dyad` on the
checked-in catalogue fixture. Supports both Relevent and REM compute interfaces.
...
```
"""
const NODSnd = _CatalogueStatistic{:NODSnd}
export NODSnd

"""
    NODRec(n_actors) <: AbstractStatistic

Receiver cumulative outdegree divided by the number of past events. Before the first event, the value is 1/(n_actors-1), matching R relevent.
Cumulative unit-event semantics agree with `relevent::rem.dyad` on the
checked-in catalogue fixture. Supports both Relevent and REM compute interfaces.
...
```
"""
const NODRec = _CatalogueStatistic{:NODRec}
export NODRec

"""
    NTDegSnd(n_actors) <: AbstractStatistic

Sender total degree divided by twice the number of past events. Before the first event, the value is 1/(n_actors-1), matching R relevent.
Cumulative unit-event semantics agree with `relevent::rem.dyad` on the
checked-in catalogue fixture. Supports both Relevent and REM compute interfaces.
...
```
"""
const NTDegSnd = _CatalogueStatistic{:NTDegSnd}
export NTDegSnd

"""
    NTDegRec(n_actors) <: AbstractStatistic

Receiver total degree divided by twice the number of past events. Before the first event, the value is 1/(n_actors-1), matching R relevent.
Cumulative unit-event semantics agree with `relevent::rem.dyad` on the
checked-in catalogue fixture. Supports both Relevent and REM compute interfaces.
...
```
"""
const NTDegRec = _CatalogueStatistic{:NTDegRec}
export NTDegRec

"""
    RRecSnd(n_actors) <: AbstractStatistic

Inverse recency rank of the receiver among the sender’s past incoming partners. Unseen partners have value zero; distinct partners are ranked by their last event order.
Cumulative unit-event semantics agree with `relevent::rem.dyad` on the
checked-in catalogue fixture. Supports both Relevent and REM compute interfaces.
...
```
"""
const RRecSnd = _CatalogueStatistic{:RRecSnd}
export RRecSnd

"""
    RSndSnd(n_actors) <: AbstractStatistic

Inverse recency rank of the receiver among the sender’s past outgoing partners. Unseen partners have value zero; distinct partners are ranked by their last event order.
Cumulative unit-event semantics agree with `relevent::rem.dyad` on the
checked-in catalogue fixture. Supports both Relevent and REM compute interfaces.
...
```
"""
const RSndSnd = _CatalogueStatistic{:RSndSnd}
export RSndSnd

"""
    OTPSnd(n_actors) <: AbstractStatistic

Sum over third actors k of min(count(s→k), count(k→r)). Counts retain repeated events, so this is not a binary two-path statistic.
Cumulative unit-event semantics agree with `relevent::rem.dyad` on the
checked-in catalogue fixture. Supports both Relevent and REM compute interfaces.
...
```
"""
const OTPSnd = _CatalogueStatistic{:OTPSnd}
export OTPSnd

"""
    ITPSnd(n_actors) <: AbstractStatistic

Sum over third actors k of min(count(r→k), count(k→s)). Counts retain repeated events, so this is not a binary two-path statistic.
Cumulative unit-event semantics agree with `relevent::rem.dyad` on the
checked-in catalogue fixture. Supports both Relevent and REM compute interfaces.
...
```
"""
const ITPSnd = _CatalogueStatistic{:ITPSnd}
export ITPSnd

"""
    ISPSnd(n_actors) <: AbstractStatistic

Sum over third actors k of min(count(k→s), count(k→r)). Counts retain repeated events, so this is not a binary two-path statistic.
Cumulative unit-event semantics agree with `relevent::rem.dyad` on the
checked-in catalogue fixture. Supports both Relevent and REM compute interfaces.
...
```
"""
const ISPSnd = _CatalogueStatistic{:ISPSnd}
export ISPSnd

name(::_CatalogueStatistic{K}) where K = string(K)

_count(acc::_DecayAccum, s::Int, r::Int) = get(acc.dyad, (s, r), (0.0, 0.0))[1]
_degree(d, actor) = get(d, actor, (0.0, 0.0))[1]

function _catalogue_compute(stat::_CatalogueStatistic{K}, events, s::Int, r::Int) where K
    n = stat.n_actors
    1 <= s <= n && 1 <= r <= n || throw(ArgumentError("candidate actor outside 1:$n"))
    acc = _accum(stat.cache, events, 0.0)
    if K in (:NIDSnd, :NIDRec, :NODSnd, :NODRec, :NTDegSnd, :NTDegRec)
        acc.cursor == 0 && return 1.0 / (n - 1)
        actor = K in (:NIDSnd, :NODSnd, :NTDegSnd) ? s : r
        incoming = _degree(acc.in_val, actor)
        outgoing = _degree(acc.out_val, actor)
        return K in (:NIDSnd, :NIDRec) ? incoming / acc.cursor :
               K in (:NODSnd, :NODRec) ? outgoing / acc.cursor :
               (incoming + outgoing) / (2acc.cursor)
    elseif K in (:RRecSnd, :RSndSnd)
        incoming = K === :RRecSnd
        index = get(acc.dyad_order, incoming ? (r, s) : (s, r), 0)
        index == 0 && return 0.0
        rank = 1
        for k in 1:n
            get(acc.dyad_order, incoming ? (k, s) : (s, k), 0) > index && (rank += 1)
        end
        return 1.0 / rank
    end
    value = 0.0
    for k in 1:n
        (k == s || k == r) && continue
        value += K === :OTPSnd ? min(_count(acc, s, k), _count(acc, k, r)) :
                 K === :ITPSnd ? min(_count(acc, r, k), _count(acc, k, s)) :
                                min(_count(acc, k, s), _count(acc, k, r))
    end
    return value
end

compute(stat::_CatalogueStatistic, h::InteractionHistory{T}, s::Int, r::Int, ::T) where T =
    _catalogue_compute(stat, h.events, s, r)
function compute(stat::_CatalogueStatistic, state::REM.EventNetworkState, s::Int, r::Int)
    state.keep_history || throw(ArgumentError("R catalogue statistics require REM event history; construct the state with keep_history=true"))
    return _catalogue_compute(stat, state.event_history, s, r)
end
...
"""
    CovEvent(values::AbstractMatrix; name="CovEvent") <: AbstractStatistic

Static directed dyad covariate `values[sender, receiver]`, matching R relevent's
CovEvent. Supply one statistic per matrix; event-indexed covariate arrays are
not supported. Both compute interfaces use actor IDs as matrix indices.

# Example
```julia
using Relevent, REM
stat = CovEvent([0.0 2.0; -1.0 0.0])
compute(stat, InteractionHistory(), 1, 2, 0.0) # 2.0
```
"""
struct CovEvent <: AbstractStatistic
    values::Matrix{Float64}
    stat_name::String
    function CovEvent(values::AbstractMatrix{<:Real}; name::String="CovEvent")
        size(values, 1) == size(values, 2) || throw(ArgumentError("CovEvent requires a square matrix"))
        all(isfinite, values) || throw(ArgumentError("CovEvent values must be finite"))
        new(Matrix{Float64}(values), name)
    end
end
export CovEvent
name(stat::CovEvent) = stat.stat_name
REM.needs_history(::CovEvent) = false
function _cov_event(stat::CovEvent, s::Int, r::Int)
    n = size(stat.values, 1)
    1 <= s <= n && 1 <= r <= n || throw(ArgumentError("candidate actor has no CovEvent value"))
    return stat.values[s, r]
end
compute(stat::CovEvent, ::InteractionHistory{T}, s::Int, r::Int, ::T) where T = _cov_event(stat, s, r)
compute(stat::CovEvent, ::REM.EventNetworkState, s::Int, r::Int) = _cov_event(stat, s, r)

struct _FixedEffect{K} <: AbstractStatistic
    actor::Int
    function _FixedEffect{K}(actor::Int) where K
        actor >= 2 || throw(ArgumentError("$K uses actor 1 as reference; choose actor >= 2"))
        new{K}(actor)
    end
end
"""
    FESnd(actor) <: AbstractStatistic

R-style fixed effect: indicator that the sender equals `actor`.
Actor 1 is the reference; add one statistic for each desired actor in `2:n`.
...
```
"""
const FESnd = _FixedEffect{:FESnd}
export FESnd
"""
    FERec(actor) <: AbstractStatistic

R-style fixed effect: indicator that the receiver equals `actor`.
Actor 1 is the reference; add one statistic for each desired actor in `2:n`.
...
```
"""
const FERec = _FixedEffect{:FERec}
export FERec
"""
    FEInt(actor) <: AbstractStatistic

R-style fixed effect: indicator that the sender or receiver equals `actor`.
Actor 1 is the reference; add one statistic for each desired actor in `2:n`.
...
```
"""
const FEInt = _FixedEffect{:FEInt}
export FEInt
name(stat::_FixedEffect{K}) where K = "$K.$(stat.actor)"
REM.needs_history(::_FixedEffect) = false
function _fixed_effect(stat::_FixedEffect{K}, s::Int, r::Int) where K
    return K === :FESnd ? Float64(s == stat.actor) :
           K === :FERec ? Float64(r == stat.actor) :
                         Float64(s == stat.actor) + Float64(r == stat.actor)
end
compute(stat::_FixedEffect, ::InteractionHistory{T}, s::Int, r::Int, ::T) where T = _fixed_effect(stat, s, r)
compute(stat::_FixedEffect, ::REM.EventNetworkState, s::Int, r::Int) = _fixed_effect(stat, s, r)
...
function _risk_set_plan(events::Vector{Event{T}}, statistics::Tuple, n_actors::Int;
    ...
    for stat in statistics
        stat isa _CatalogueStatistic && stat.n_actors != n_actors &&
            throw(ArgumentError("$(name(stat)) was constructed for $(stat.n_actors) actors but the fit uses $n_actors"))
        stat isa _FixedEffect && stat.actor > n_actors &&
            throw(ArgumentError("$(name(stat)) refers to an actor outside the fit's 1:$n_actors universe"))
    end
...
# =============================================================================
#
# Both are `θ -> (ll, grad, hess)` for `newton_fit`, and both run on workspaces
...
# A Newton objective tolerance does not establish existence of a finite MLE.
# Search only a few candidate rays, then prove every required sign using exact
# arithmetic on the Float64 design. A failed search is not a proof of overlap.
# Screening tolerances can only trigger exact verification; they never turn a
# negative contrast into a separating one. Each pass retains bounded cache use.
function _separation_screen(rs::_RiskSets, direction; timing::Bool)
    valid = Ref(true)
    reference = Ref{Union{Nothing,Vector{Float64}}}(nothing)
    p = rs.plan.p
    each_interval(rs) do m, X, weights
        valid[] || return
        ci = rs.plan.case_idx[m]
        if timing && reference[] === nothing && ci > 0
            reference[] = copy(X[ci, :])
        end
        anchor = timing ? reference[] : (ci > 0 ? view(X, ci, :) : nothing)
        anchor === nothing && (valid[] = false; return)
        for j in axes(X, 1)
            event = timing && j == ci
            exposure = timing && rs.plan.waiting[m] > 0
            competitor = !timing && (weights === nothing || weights[j] > 0)
            event || exposure || competitor || continue
            margin, scale = 0.0, 0.0
            for k in 1:p
                a, b = anchor[k], X[j, k]
                isfinite(a) && isfinite(b) || (valid[] = false; return)
                margin += (a - b) * direction[k]
                scale += (abs(a) + abs(b)) * abs(direction[k])
            end
            tolerance = 64 * eps(Float64) * max(p, 1) * scale
            # Timing events must have zero joint predictor along the ray;
            # exposures require anchor-other >= 0, like ordinal competitors.
            if margin < -tolerance || (event && margin > tolerance)
                valid[] = false
                return
            end
        end
...
function _exact_separation_certificate(rs::_RiskSets, direction; timing::Bool)
    Q = Rational{BigInt}
    ray = Q.(direction)
    valid, strict = Ref(true), Ref(false)
    reference = Ref{Union{Nothing,Q}}(nothing)
    each_interval(rs) do m, X, weights
        valid[] || return
        ci = rs.plan.case_idx[m]
        # For timing choose the baseline component EXACTLY as minus the first
        # event's effect score. This constructs a candidate ray; every other
        # event and every positive exposure are still verified without tolerance.
        if timing && reference[] === nothing && ci > 0
            all(isfinite, view(X, ci, :)) || (valid[] = false; return)
            reference[] = sum(Q(X[ci, k]) * ray[k] for k in eachindex(ray); init=zero(Q))
        end
        if !timing && ci > 0
            all(isfinite, view(X, ci, :)) || (valid[] = false; return)
        end
        anchor = timing ? reference[] : (ci > 0 ?
            sum(Q(X[ci, k]) * ray[k] for k in eachindex(ray); init=zero(Q)) : nothing)
        anchor === nothing && (valid[] = false; return)
        ...
        for j in axes(X, 1)
            ...
            competitor = !timing && (weights === nothing || weights[j] > 0)
            event || exposure || competitor || continue
            all(isfinite, view(X, j, :)) || (valid[] = false; return)
            score = sum(Q(X[j, k]) * ray[k] for k in eachindex(ray); init=zero(Q))
            margin = anchor - score
            if margin < 0 || (event && margin != 0)
                valid[] = false
                return
            end
            strict[] |= margin > 0 && (exposure || competitor)
            positive_weight |= competitor
        end
        !timing && !positive_weight && (valid[] = false)
    end
    return valid[] && strict[]
end

function _separation_certificate(rs::_RiskSets, direction; timing::Bool=false)
    length(direction) == rs.plan.p || throw(DimensionMismatch("separation direction must match the effects"))
    all(isfinite, direction) || return false
    magnitude = maximum(abs, direction; init=0.0)
    magnitude > 0 || return false
    ray = direction ./ magnitude
    _separation_screen(rs, ray; timing) || return false
    return _exact_separation_certificate(rs, ray; timing)
end

function _reject_verified_separation(rs::_RiskSets, opt, derivatives; timing::Bool=false)
    _, gradient, hessian = derivatives(opt.θ)
    step = try
        -(hessian \ gradient)
    catch err
        err isa Union{SingularException,LAPACKException,ZeroPivotException,PosDefException} || rethrow()
        nothing
    end
    for candidate in (step, opt.θ)
        candidate === nothing && continue
        direction = timing ? candidate[2:end] : candidate
        if _separation_certificate(rs, direction; timing)
            fitter = timing ? "fit_timing" : "fit_obpm"
            throw(ArgumentError("$fitter: verified separation; no finite maximum-likelihood " *
                                "estimate exists for this model. An exactly checked direction " *
                                "strictly improves the likelihood without a finite optimum. " *
                                "Remove or revise the separating effects; more Newton iterations " *
                                "cannot produce finite identified estimates."))
        end
    end
    return nothing
end
...
sampling — `REM.fit_rem` also offers sampled-risk-set estimation).
"""
struct OrdinalBPM
...
# distinguishing property against sampled-control `REM.fit_rem` fits — so
...
After optimization, candidate directions from the final Newton step and fitted
coefficients are checked for separation. An exactly verified direction that
improves the likelihood indefinitely raises `ArgumentError`: no finite MLE
exists. The check covers complete and quasi separation when a candidate is
found; it is not an exhaustive linear-programming search. `converged=true` alone
is therefore not a proof that all possible boundary directions were excluded.
Floating-point screening is followed by exact rational verification of every
positive-weight case-versus-competitor contrast; tiny genuine overlap is never
rounded into a separation certificate.
...
This likelihood models **event choice conditional on the observed history**,
without a waiting-time likelihood. Finite-half-life statistics still use the
observed time differences; when only order is available, choose order-based
statistics such as participation shifts or cumulative counts. A tied timestamp
means the event order is unobserved, and sorting the tie invents it (worse: the statistics are read off
...
function fit_obpm(events::Vector{Event{T}}, statistics::Vector{<:AbstractStatistic},
    ...
    opt = newton_fit(derivatives, zeros(p); maxiter=maxiter, tol=tol)
    _reject_verified_separation(rs, opt, derivatives)
    opt.converged || @warn "fit_obpm did not converge; inspect the model for non-identification or separation, or increase maxiter" iterations=opt.iterations
    return OrdinalBPMResult(model, opt.θ, opt.se, opt.loglik, opt.converged,
...
"""
    is_interval_constant(stat::AbstractStatistic) -> Bool

Whether `compute(stat, history, sender, receiver, t)` is constant as `t`
advances between events with the history and candidate dyad fixed. This is
required by [`fit_timing`](@ref), whose exposure is waiting time times hazard.
The default is `false`: custom statistics must explicitly extend this function
to return `true` only when that identity holds for every admissible history.
The trait is public, accessed as `Relevent.is_interval_constant`.

Participation shifts, cumulative R catalogue statistics, fixed effects and
static covariates satisfy the contract. `PriorInteraction`, `SendingCapacity`,
`ReceivingCapacity`, `LocalInertia` and `Momentum` satisfy it only with positive
`halflife=Inf` (zero decay). Finite-half-life statistics remain available for
ordinal/conditional estimation and direct computation; their continuously
varying hazard is not integrated by the timing fitter.

# Example
```julia
using Relevent
Relevent.is_interval_constant(PShift(:AB_BA))        # true
Relevent.is_interval_constant(PriorInteraction(Inf)) # true
Relevent.is_interval_constant(PriorInteraction(2.0)) # false
```
"""
is_interval_constant(::AbstractStatistic) = false
is_interval_constant(::Union{PShift,_CatalogueStatistic,_FixedEffect,
                              CovSnd,CovRec,CovInt,CovEvent}) = true
is_interval_constant(stat::Union{PriorInteraction,SendingCapacity,
                                 ReceivingCapacity,LocalInertia,Momentum}) =
    stat.halflife == Inf
...
the exponential-baseline proportional-hazards process with statistics constant
between events. [`fit_timing`](@ref) enforces [`is_interval_constant`](@ref);
finite-half-life statistics and uncertified custom statistics are rejected
because their hazard would require integration over the waiting interval.
Weibull/Gompertz baselines are not fitted.
...
All fitted log-rate coefficients, ordered `[log(λ₀); θ]`. The effect-only
vector remains in `result.coefficients`, and `result.baseline_params[1]`
is the baseline on the rate scale. This includes the baseline since 0.2.0.
"""
...
Standard errors in the same `[log(λ₀); θ]` order as `coef(result)`.
"""
...
Every statistic must satisfy [`is_interval_constant`](@ref). Supported built-ins
are `PShift`, cumulative R catalogue statistics, fixed effects and static
`CovSnd`/`CovRec`/`CovInt`/`CovEvent`. The five half-life statistics are supported
only with `halflife=Inf`, which specifies zero decay. Finite-half-life and
uncertified custom statistics raise `ArgumentError`: multiplying the hazard
evaluated at an event endpoint by the preceding interval is not its integral.
Use [`fit_obpm`](@ref) for their conditional event-choice likelihood, or define
an interval-constant model deliberately; changing the half-life changes the model.

A verified separating direction raises `ArgumentError` instead of returning
finite estimates where no finite MLE exists. The timing certificate requires
zero change in every observed event's joint predictor, nonpositive change for
every positive-exposure predictor, and at least one strict exposure decrease.
It includes censored tails and retains event constraints at zero waiting times.
Candidates come from the final Newton step and coefficients, with the baseline
direction reconstructed exactly from the first event; all signs and event
equalities are verified using rational arithmetic on the stored design values.
This conservative search is not exhaustive; absence of a certificate does not
prove that a finite MLE exists.
...
function fit_timing(events::Vector{Event{T}}, statistics::Vector{<:AbstractStatistic},
    ...
    for stat in model.statistics
        is_interval_constant(stat) || throw(ArgumentError(
            ...
            "$(typeof(stat)) does not satisfy Relevent.is_interval_constant. " *
            "Continuously varying hazards require exposure integration, which " *
            "this fitter does not implement. Use fit_obpm for conditional " *
            "event choice, or explicitly specify an interval-constant statistic " *
            "(halflife=Inf means zero decay and changes the model)."))
    end
    ...
    opt = newton_fit(derivatives, zeros(p + 1); maxiter=maxiter, tol=tol)
    _reject_verified_separation(rs, opt, derivatives; timing=true)
    opt.converged || @warn "fit_timing did not converge; inspect the model for non-identification or separation, or increase maxiter" iterations=opt.iterations
    β = opt.θ
    return TimingModelResult(model, β[2:end], [exp(β[1])], opt.se[2:end],
                             opt.loglik, opt.converged, tie_applied, β[1],
                             opt.se[1], opt.vcov, length(events), opt.iterations)
end

# Common StatsAPI surface. The timing baseline is an estimated coefficient,
# included consistently in every vector, matrix, table and parameter count.
const _ReleventResult = Union{OrdinalBPMResult,TimingModelResult}

"""
    coefnames(result)

Coefficient names in `coef` order; timing fits start with `"log_baseline"`.
"""
coefnames(result::OrdinalBPMResult) = [name(s) for s in result.model.statistics]
coefnames(result::TimingModelResult) = ["log_baseline"; [name(s) for s in result.model.statistics]]

"""
    vcov(result)

Full observed-information covariance, in `coef` order. Non-positive-definite
information yields `NaN` throughout with a fitting warning, not a pseudoinverse.
"""
vcov(result::_ReleventResult) = result.var_cov

"""
    loglikelihood(result)

The fitted ordinal partial or interval log likelihood, including censored exposure.
"""
loglikelihood(result::_ReleventResult) = result.loglik

"""
    nobs(result)

Number of observed events. A right-censored tail is exposure, not another event.
"""
nobs(result::_ReleventResult) = result.n_events

"""
    dof(result)

Number of estimated coefficients, including the timing log baseline.
"""
dof(result::_ReleventResult) = length(coef(result))

"""
    aic(result)

`-2loglikelihood(result) + 2dof(result)`. Compare fits using the same events,
observation window and likelihood (ordinal and interval AICs are not comparable).
"""
aic(result::_ReleventResult) = -2loglikelihood(result) + 2dof(result)

"""
    aicc(result)

Small-sample AIC using observed events as the sample size. Returns `Inf` when
`nobs(result) <= dof(result) + 1`.
"""
function aicc(result::_ReleventResult)
...
`-2loglikelihood(result) + log(nobs(result))*dof(result)`, using event count.
"""
bic(result::_ReleventResult) = -2loglikelihood(result) + log(nobs(result)) * dof(result)

"""
    confint(result; level=0.95)

Two-column Wald confidence intervals in `coef` order and on the log-rate scale.
`level` must be strictly between zero and one. Unavailable uncertainty remains NaN.
"""
function confint(result::_ReleventResult; level::Real=0.95)
...
An inspectable `Networks.CoefficientTable`, including baseline uncertainty for
interval fits. Wald p-values use the shared, NaN-aware, floored `z_pvalues`.
"""
function coeftable(result::_ReleventResult)
    c, se = coef(result), stderror(result)
    z, p = z_pvalues(c, se)
...
With `ordinal=true` (the default, as in `rem.dyad`) event choices are modeled
conditional on the history; elapsed times still enter finite-half-life statistics and the model is fitted by [`fit_obpm`](@ref), returning an

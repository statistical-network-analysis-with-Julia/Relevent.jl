...
Relevent.jl provides advanced statistics for relational event models that complement the basic statistics in REM.jl. The original history statistics use half-life decay. The cumulative R catalogue, participation shifts, and static covariates have their own conventions; see [R effect coverage](r_catalogue.md).

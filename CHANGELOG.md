...
surface is extended (13 Gibson p-shifts, `CovSnd`/`CovRec`/`CovInt`,
...
- Both fitters now throw `ArgumentError` for an exactly verified separating
  direction proving no finite MLE exists. Previously, complete/quasi separation
  could produce large finite coefficients and standard errors with
  `converged=true` and no caveat. Candidate final Newton/coefficient directions
  undergo exact rational verification after floating-point screening; tiny true
  overlap is preserved. Timing certificates include all observed events and
  positive exposure, including censored tails. The search is conservative and
  nonexhaustive; it does not claim to detect every separating direction.

- `fit_timing` now rejects finite-half-life and uncertified custom statistics.
  Their hazard changes during a waiting interval; the previous implementation
  used its endpoint value for the entire exposure and incorrectly advertised an
  exact likelihood. Public `Relevent.is_interval_constant` provides an explicit
  extension contract. Participation shifts, cumulative R catalogue effects,
  fixed effects and static covariates remain supported. Positive `halflife=Inf`
  permits deliberate zero-decay models; finite decay remains available for
  ordinal/conditional estimation. Timing examples and the parameter-recovery
  test now use correctly specified interval-constant models.

- Timing StatsAPI `coef` and `stderror` now include the estimated log baseline
  first, consistently with new `vcov`, confidence intervals and coefficient
  tables. Existing `.coefficients`/`.std_errors` fields remain effect-only.
  Both result structs now retain covariance and iteration metadata; timing
  results also retain event count and baseline uncertainty.
- Replace the local Newton loop with `Networks.newton_fit`: exhausted line
  searches retain a coherent accepted point; singular information returns
  NaN uncertainty, and non-convergence warns. The ordinal covariance uses
  centered moments so a constant statistic has exactly zero information.
- Timing exposure exponentiates the joint log baseline, predictor and interval
  length, preserving finite hazards when individual exponentials overflow.
  Zero waiting intervals contribute exactly zero exposure.
- `rem_dyad` is a true const alias of `fit_relevent`.
- Fitters reject nonpositive iteration limits, invalid tolerances, nonfinite
  observation-window endpoints, and catalogue actor-universe mismatches.
...
### September quality pass

- Add normalized degree, inverse recency rank, OTPSnd/ITPSnd/ISPSnd,
  FESnd/FERec/FEInt, and static CovEvent constructors with both compute
  interfaces. Add a reproducible R 1.2.1 design fixture with 24 columns,
  all candidate dyads, repeated interactions and an isolated actor.
- Document exact unsupported R effects and reference discrepancies,
  time-varying covariates, Bayesian fitting, and risk-set limitations.
- Complete the StatsAPI result surface and use the shared coefficient table
  and floored Wald p-values. Information criteria count the timing baseline.
- Repair benchmark source paths; add bounded allocation gates with four-thread
  CI. CI reconstructs dependency checkouts from project metadata and records
  sibling SHAs. Documentation remains strict.

...
REM.name(::TimingUncertifiedStatistic) = "uncertified"
REM.name(::TimingCertifiedStatistic) = "certified_sender"
REM.compute(::TimingUncertifiedStatistic, ::InteractionHistory, s::Int, r::Int, t) = t
REM.compute(::TimingCertifiedStatistic, ::InteractionHistory, s::Int, r::Int, t) = Float64(s - 1)
Relevent.is_interval_constant(::TimingCertifiedStatistic) = true
...
@testset "Relevent.jl" begin
    ...
    @testset "fit_timing recovers rate and coefficient" begin
        ...
        λ0_true = 0.4
        θ_true = 0.9
        stat = CovSnd(collect(range(-1.0, 1.0; length=n)))
        dyads = [(s, r) for s in 1:n for r in 1:n if s != r]
        h = InteractionHistory{Float64}()
        events = Event{Float64}[]
        t = 0.0
        ...
        for m in 1:500
            ...
            # The static statistic is unchanged throughout this waiting interval.
            probs = w ./ sum(w)
            u = rand(rng)
            acc = 0.0
            pick = length(dyads)
    ...
    @testset "Risk-set cache policies (:all / :chunked / :none)" begin
        ...
        timing_stats = [PriorInteraction(Inf), PShift(:AB_BA), SendingCapacity(Inf)]
        ref_o = fit_obpm(events, stats, n)
        ref_t = fit_timing(events, timing_stats, n; t_end=t + 5.0)
        ...
        for cache in (:auto, :all, :chunked, :none), chunk in (nothing, 1, 3, 17, 500)
            ...
            tm = fit_timing(events, timing_stats, n; t_end=t + 5.0, cache=cache, chunk=chunk)
    ...
    @testset "fit_relevent / rem_dyad standardized entry point" begin
        ...
        timing_stats = [PShift(:AB_BA), LocalInertia(Inf)]
        r_tim = fit_relevent(events, timing_stats, n; ordinal=false, t0=0.0)
        @test r_tim isa TimingModelResult
        ref_tim = fit_timing(events, timing_stats, n; t0=0.0)
    ...
    @testset "Result metadata protocol" begin
        ...
        tim = fit_timing(events, [PriorInteraction(Inf)], n)
    ...
    @testset "Tied event times: the two likelihoods take different policies" begin
        ...
        stats = [PShift(:AB_BA), PriorInteraction(Inf)]
        ...
        # Breslow accepts duplicated dyads, but this tiny two-effect example
        # has no finite MLE. Separate tie admissibility from estimability.
        e_breslow = try; fit_obpm(dup, stats, n; ties=:breslow); catch e; e; end
        @test e_breslow isa ArgumentError
        @test occursin("verified separation", sprint(showerror, e_breslow))
        @test fit_obpm(dup, [CovSnd([0., 1., 2., 3.])], n; ties=:breslow).converged
...
@testset "Shared optimizer and coherent result protocol" begin
    @test Relevent.newton_fit === Networks.newton_fit
    @test Relevent.rem_dyad === Relevent.fit_relevent
    @test !isdefined(Relevent, :_newton)
    # The old final halving bug returned θ and ll from different points.
    q(x) = (-exp(-1e-4*x[1])-x[1], [1e-4*exp(-1e-4*x[1])-1],
            reshape([-1e-8*exp(-1e-4*x[1])],1,1))
    opt = Relevent.newton_fit(q, [0.0]; maxiter=1)
    @test opt.loglik == q(opt.θ)[1]
    @test !opt.converged
    g = Networks.load_golden(joinpath(@__DIR__, "fixtures", "relevent_rem_dyad.toml"))
    v = g.values
    ...
    stats = [CovSnd(z), CovRec(z)]
    n = Int(v["n_actors"])
    for timing in (false, true)
        fit = timing ? fit_timing(events, stats, n; t_end=Float64(v["t_end"])) :
                       fit_obpm(events, stats, n)
        @test fit.converged
        ...
        @test coeftable(fit).p_values == Networks.z_pvalues(coef(fit),stderror(fit)).p
        @test occursin(coefnames(fit)[1], sprint(show, fit))
        plan = Relevent._risk_set_plan(events, stats, n;
                    t_end=timing ? Float64(v["t_end"]) : nothing)
        rs = Relevent._risk_sets(plan)
        deriv = timing ? Relevent._timing_derivatives(rs) : Relevent._obpm_derivatives(rs)
        ll, grad, hess = deriv(coef(fit))
        @test ll == fit.loglik
        @test vcov(fit) ≈ inv(-hess)
        scaled = timing ? fit_timing(events, [CovSnd(1e8 .* z),CovRec(z)], n;
                                    t_end=Float64(v["t_end"])) :
                          fit_obpm(events, [CovSnd(1e8 .* z),CovRec(z)], n)
        ...
        @test_logs (:warn, r"did not converge") begin
            stalled = timing ? fit_timing(events, stats, n; maxiter=1) :
                               fit_obpm(events, stats, n; maxiter=1)
            @test !stalled.converged
        end
    end
    @test_logs (:warn, r"not negative definite") (:warn, r"did not converge") begin
        singular = fit_obpm(events, [CovSnd(ones(n))], n)
...
@testset "Stable joint timing predictor and analytic derivatives" begin
    events = [Event(1,2,1.0),Event(2,3,2.0),Event(3,1,3.0)]
    plan = Relevent._risk_set_plan(events,[CovSnd(ones(3))],3)
    f = Relevent._timing_derivatives(Relevent._risk_sets(plan))
    @test f([0.0,0.0])[1] == -18.0
    @test f([-1000.0,1000.0]) == f([0.0,0.0])
    @test f([1000.0,-1000.0]) == f([0.0,0.0])
    # Zero exposure with an overflowing hazard is exactly zero, not Inf*0.
    plan0 = Relevent._risk_set_plan([Event(1,2,0.0)], [CovSnd(ones(3))], 3)
    f0 = Relevent._timing_derivatives(Relevent._risk_sets(plan0))
    ...
    ft = Relevent._timing_derivatives(Relevent._risk_sets(
         Relevent._risk_set_plan(tiny,[CovSnd(ones(3))],3)))
    @test all(isfinite, ft([710.0,0.0])[2])
    rs = Relevent._risk_sets(Relevent._risk_set_plan(events,
         [CovSnd([-0.5,0.1,0.7]),CovRec([0.4,-0.2,0.8])],3; t_end=4.5))
    f = Relevent._timing_derivatives(rs)
    b = [-1.2,0.3,-0.4]
    ll, grad, hess = f(b)
    eps = 1e-5
...
@testset "R relevent catalogue design parity" begin
    g = Networks.load_golden(joinpath(@__DIR__,"fixtures","relevent_catalogue.toml"))
    n = Int(g.values["n_actors"])
    events = [Event(Int(s),Int(r),Float64(t)) for (s,r,t) in zip(
              g.values["input_sender"],g.values["input_receiver"],g.values["input_time"])]
    stats = AbstractStatistic[NIDSnd(n),NIDRec(n),NODSnd(n),NODRec(n),
               NTDegSnd(n),NTDegRec(n),RRecSnd(n),RSndSnd(n),OTPSnd(n),ITPSnd(n),ISPSnd(n)]
    append!(stats,[FESnd(i) for i in 2:n])
    append!(stats,[FERec(i) for i in 2:n])
    append!(stats,[FEInt(i) for i in 2:n])
    push!(stats,CovEvent([(3i-j)/7 for i in 1:n,j in 1:n]))
    for stat in stats
        ...
        for event in events
            for s in 1:n,r in 1:n
                s == r && continue
                value = compute(stat,history,s,r,event.time)
                push!(actual,value)
                @test value == compute(stat,state,s,r)
            end
            update_history!(history,event)
            REM.update!(state,event)
        end
        key = replace(Relevent.name(stat), "." => "_")
        @test Networks.check_golden(g,key,actual)
    end
    @test_throws ArgumentError NIDSnd(1)
    @test_throws ArgumentError FESnd(1)
    @test_throws ArgumentError CovEvent(ones(2,3))
    @test_throws ArgumentError CovEvent(fill(NaN,2,2))
    @test_throws ArgumentError compute(CovEvent(ones(2,2)),InteractionHistory(),1,3,0.0)
end

@testset "Fit and catalogue validation" begin
    ev = [Event(1,2,1.0),Event(2,3,2.0)]
    @test_throws ArgumentError fit_obpm(ev,[NIDSnd(4)],3)
    @test_throws ArgumentError fit_obpm(ev,[FESnd(4)],3)
    for fitfun in (fit_obpm,fit_timing)
        @test_throws ArgumentError fitfun(ev,[CovSnd([1.,2.,3.])],3; maxiter=0)
        @test_throws ArgumentError fitfun(ev,[CovSnd([1.,2.,3.])],3; tol=NaN)
    end
    @test_throws ArgumentError fit_timing(ev,[CovSnd([1.,2.,3.])],3; t0=-Inf)
    @test_throws ArgumentError fit_timing(ev,[CovSnd([1.,2.,3.])],3; t_end=Inf)
end

@testset "Exact timing requires interval-constant statistics" begin
    events = [Event(1,2,1.0), Event(2,1,3.0)]
    history = history_fixture()
    @test Base.ispublic(Relevent, :is_interval_constant)
    for constructor in (PriorInteraction, SendingCapacity, ReceivingCapacity,
                        LocalInertia, Momentum)
        decayed = constructor(1.0)
        cumulative = constructor(Inf)
        @test !Relevent.is_interval_constant(decayed)
        @test Relevent.is_interval_constant(cumulative)
        ...
        for cache in (:all, :chunked, :none)
            err = try
                fit_timing(events, [decayed], 2; cache=cache)
            catch exception
                exception
            end
        ...
        @test_throws ArgumentError fit_relevent(events, [decayed], 2; ordinal=false)
        @test_throws ArgumentError rem_dyad(events, [decayed], 2; ordinal=false)
    end
    @test Relevent.is_interval_constant(Momentum(Inf; normalize=true))
    @test !Relevent.is_interval_constant(Momentum(-Inf))
    @test !Relevent.is_interval_constant(Momentum(NaN))
    for constructor in (NIDSnd,NIDRec,NODSnd,NODRec,NTDegSnd,NTDegRec,
                        RRecSnd,RSndSnd,OTPSnd,ITPSnd,ISPSnd)
        @test Relevent.is_interval_constant(constructor(3))
    end
    for stat in (PShift(:AB_BA), FESnd(2), FERec(2), FEInt(2),
                 CovSnd([1.,2.,3.]), CovRec([1.,2.,3.]), CovInt([1.,2.,3.]),
                 CovEvent(zeros(3,3)))
        @test Relevent.is_interval_constant(stat)
    end
    @test !Relevent.is_interval_constant(TimingUncertifiedStatistic())
    @test_throws ArgumentError fit_timing(events, [TimingUncertifiedStatistic()], 2)
    balanced = [Event(1,2,1.),Event(2,3,2.),Event(3,1,3.),
                Event(1,3,4.),Event(2,1,5.),Event(3,2,6.)]
    custom = fit_timing(balanced, [TimingCertifiedStatistic()], 3; t_end=7.)
    built_in = fit_timing(balanced, [CovSnd([0.,1.,2.])], 3; t_end=7.)
    @test custom.converged && built_in.converged
    @test coef(custom) == coef(built_in)
    @test vcov(custom) == vcov(built_in)
    @test Networks.is_exact(custom)

    # With no decay the true hazard is constant on [1,3]. Its integrated
    # exposure is exactly 2*(exp(1)+1), following initial exposure2.
    rs = Relevent._risk_sets(Relevent._risk_set_plan(events, [PriorInteraction(Inf)], 2))
    @test Relevent._timing_derivatives(rs)([0.,1.])[1] ≈ -2 - 2*(exp(1)+1)
    # Include both repeated and previously unobserved dyads: a sequence with
    # only first occurrences separates a repetition effect toward -Inf.
    overlapping_history = [balanced; Event(3,2,7.); Event(1,2,8.)]
    @test fit_obpm(overlapping_history, [PriorInteraction(1.0)], 3).converged
end

@testset "Verified separation rejects a nonexistent finite MLE" begin
    ...
    for (events, stat) in ((quasi, CovSnd([0.,1.,2.])), (complete, CovEvent(dyad_x)))
        for cache in (:all, :chunked, :none), timing in (false, true)
            err = try
                timing ? fit_timing(events, [stat], 3; t_end=13.0, cache=cache, chunk=2) :
                         fit_obpm(events, [stat], 3; cache=cache, chunk=2)
            catch exception
                exception
            end
            @test err isa ArgumentError
            @test occursin("verified separation", sprint(showerror, err))
            @test occursin("no finite maximum-likelihood", sprint(showerror, err))
        end
    end
    @test_throws ArgumentError fit_relevent(quasi, [CovSnd([0.,1.,2.])], 3)
    @test_throws ArgumentError rem_dyad(quasi, [CovSnd([0.,1.,2.])], 3; ordinal=false)

    # Genuine overlap as small as one representable increment must not become
    # a false certificate. A tolerance is only a screen, never proof of a sign.
    for scale in (1e-8, 1.0, 1e8), timing in (false, true)
        overlap = [0., nextfloat(1.), 1.] .* scale
        plan = Relevent._risk_set_plan(quasi, [CovSnd(overlap)], 3; t_end=13.0)
        rs = Relevent._risk_sets(plan)
        # Ordinal risk sets exclude the censored tail.
        timing || (rs = Relevent._risk_sets(Relevent._risk_set_plan(quasi, [CovSnd(overlap)], 3)))
        @test !Relevent._separation_certificate(rs, [1.0]; timing)
        @test !Relevent._separation_certificate(rs, [-1.0]; timing)
    end
    # An exactly constant statistic gives a flat ray, not a strict improvement.
    for timing in (false, true)
        rs = Relevent._risk_sets(Relevent._risk_set_plan(quasi, [CovSnd(ones(3))], 3))
        @test !Relevent._separation_certificate(rs, [1.0]; timing)
        @test !Relevent._separation_certificate(rs, [0.0]; timing)
        @test !Relevent._separation_certificate(rs, [NaN]; timing)
    end

    # Tied case weights all remain positive under Efron. The same proof applies
    # to the selected tied objective; a zero-length timing interval has an event
    # constraint but adds no exposure constraint.
    tied = [Event(3,1,1.0),Event(3,2,1.0),Event(3,1,2.0),Event(3,2,2.0)]
    for policy in (:ordered, :breslow, :efron)
        @test_throws ArgumentError fit_obpm(tied, [CovSnd([0.,1.,2.])], 3; ties=policy)
    end
    for policy in (:ordered, :batch)
        @test_throws ArgumentError fit_timing(tied, [CovSnd([0.,1.,2.])], 3; ties=policy, t_end=3.)
    end
    # The censored tail is the ONLY exposure here. Without it the conservative
    # timing certificate cannot claim its required strict exposure decrease.
    instant = [Event(3,1,0.0)]
    for (finish, expected) in ((nothing, false), (1.0, true))
        rs = Relevent._risk_sets(Relevent._risk_set_plan(instant, [CovSnd([0.,1.,2.])], 3; t_end=finish))
        @test Relevent._separation_certificate(rs, [1.0]; timing=true) == expected
    end
    # A positive tail predictor invalidates a candidate even if all event
    # intervals alone would certify it. Modify only the tail design.
    rs = Relevent._risk_sets(Relevent._risk_set_plan(quasi, [CovSnd([0.,1.,2.])], 3; t_end=13.0))
    rs.X[end][1,1] = 3.0
    @test !Relevent._separation_certificate(rs, [1.0]; timing=true)

    # Seeded overlapping data remain identified and cache-identical. Each
    # sender appears equally often, so the static-covariate MLE is theta=0.
    rng = Xoshiro(20260914)
    dyads = shuffle(rng, [(s,r) for s in 1:3 for r in 1:3 if s != r])
    balanced = [Event(s,r,Float64(k)) for (k,(s,r)) in enumerate(dyads)]
    for timing in (false, true)
        fits = [timing ? fit_timing(balanced, [CovSnd([0.,1.,2.])], 3; t_end=7., cache=cache, chunk=2) :
                        fit_obpm(balanced, [CovSnd([0.,1.,2.])], 3; cache=cache, chunk=2)
                for cache in (:all, :chunked, :none)]

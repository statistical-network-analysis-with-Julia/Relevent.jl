...
3. **Ordinal Butts-Park Model** -- `fit_obpm()`: exact multinomial partial likelihood over the FULL risk set (P(event) = exp(θ'x_case)/Σ_dyads exp(θ'x)), `Networks.newton_fit`, centered observed-information covariance and SEs. Complements REM.fit_rem (which samples the risk set). Tied times: `ties=:error` (default) | `:ordered` | `:breslow` | `:efron` — see below.
...
4b. **Standardized entry point** -- `fit_relevent(events, statistics, n_actors; ordinal=true, kwargs...)` dispatches to `fit_obpm` (ordinal) or `fit_timing` (interval, forwarding `t0`); `rem_dyad` is a true const alias, not an adapter for R arguments or Bayesian defaults.
...
`_obpm_derivatives(rs)` and `_timing_derivatives(rs)` are **named** functions returning the `θ -> (ll, grad, hess)` closures for `Networks.newton_fit`, not closures buried inside the fitters — so the `@allocated` regression test measures the code that runs, not a copy of it. They allocate the `p` gradient and `p×p` Hessian they return and **nothing else**: measured at 208 bytes (ordinal, p=2) and 240 bytes (timing, p=2 plus baseline), independent of `E` and of the risk-set size. `fit_timing` previously computed `Sxx = Xm' * (w .* Xm)` per interval, allocating a full weighted copy of the risk-set design matrix *and* a fresh `p×p` outer product on every event of every Newton evaluation. Do not reintroduce a temporary in these loops; the test will catch it, and it is there to.
...
## September contracts and R catalogue

Both estimators call `Networks.newton_fit` directly. They retain full covariance
and iterations; unconverged fits warn, and singular information returns NaN
uncertainty. Ordinal moments are centered before covariance accumulation. Timing
exposure exponentiates `(log_baseline + predictor) + log(waiting)` and skips
zero waiting exposure. The timing StatsAPI vector begins with log baseline,
while the `.coefficients`/`.std_errors` fields remain effect-only. Every StatsAPI
vector, covariance, interval, coefficient name and table shares this order.
`nobs` counts events; `dof` includes the baseline.

Fifteen additional constructors are validated by `relevent_catalogue.toml`
(24 design columns). The cumulative catalogue reads retained unit-event history
and does not inherit REM decay/window/weight semantics. Counts reuse the
streaming accumulator; recency keeps last event indices, so ties respect order.
See `docs/src/guide/r_catalogue.md` for all R conventions and unsupported effects.
The fixture proves design parity on its input, not all fitted-effect parity.

Run `julia --project=benchmark benchmark/regression_tests.jl` for bounded
allocation gates. CI source closure comes from `.github/scripts/checkout_sources.jl`
(root tests plus docs/benchmark deps, then transitive hard deps), with SHAs
recorded. `--check` audits existing checkouts without cloning.

## Exact timing admissibility (independent September statistics review)

`fit_timing` enforces public `Relevent.is_interval_constant(stat)` before
constructing its risk sets. The default for unknown statistics is false.
Supported types are PShift, _CatalogueStatistic (all11 R aliases), _FixedEffect
(FESnd/FERec/FEInt), CovSnd/CovRec/CovInt/CovEvent; all five half-life statistics
are admissible only with positive halflife=Inf (zero decay). A custom method is
an explicit mathematical promise that compute is independent of time at fixed
history/dyad. Preserve ordinal/REM conditional use of finite decay.

Do not re-enable endpoint-frozen finite decay: with events1→2 at1 and2→1 at3,
PriorInteraction(1), beta=[0,1], the old ll was -6.568050833375483 but integration
of the actual hazard gives -7.516801552952518. A future continuously varying
hazard fitter must integrate exposure and its derivatives; metadata alone cannot
turn the old expression into a likelihood. Zero decay changes the specified
model and must be explained when migrating examples. The independent report is
/tmp/statnet-quality-resume/relevent-statistics-review.md.

## Verified separation guard

Both fitters call `_reject_verified_separation` after shared Newton optimization.
The candidates are the final Newton increment and coefficients; timing uses their
effect coordinates and reconstructs the baseline ray exactly from the first
observed event. `_separation_certificate(rs, direction; timing=false)` takes an
effect-only direction. A float screen can discard implausible candidates; it
cannot certify one. `_exact_separation_certificate` converts raw design entries
to Rational{BigInt} before subtraction/dot products and verifies all constraints.

Ordinal: case minus every positive-weight competitor has nonnegative directional
score, with a strict contrast somewhere. Timing: every event has zero joint
predictor change, every positive waiting-time exposure has nonpositive change,
and some exposure strictly decreases. Censored tails count; zero-duration events
still constrain the ray. Only a proved direction causes ArgumentError. There is
no coefficient/SE cutoff and no claim of exhaustive separation detection. Preserve
tiny true overlap, valid R fixtures, ties and bounded risk-set caching. Derivative
workspaces and numerical tolerances are unchanged.

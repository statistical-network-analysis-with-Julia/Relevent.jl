...
2. **Advanced REM Statistics** -- `PriorInteraction`, `SendingCapacity`, `ReceivingCapacity`, `LocalInertia`, `Momentum` subtype `AbstractStatistic` and extend the REM generics via `import REM: compute, name`. Each implements BOTH interfaces: 5-arg `compute(stat, history::InteractionHistory, s, r, t)` (used by this package's full-risk-set estimators) and 4-arg `compute(stat, state::REM.NetworkState, s, r)` (so they work inside `REM.fit_rem`). The two agree exactly (tested). Momentum's `normalize` divides by the sender's own event count.
...
- Statistic types are immutable structs subtyping `AbstractStatistic` with BOTH compute signatures (history 5-arg and REM.NetworkState 4-arg) plus `name(stat)`.

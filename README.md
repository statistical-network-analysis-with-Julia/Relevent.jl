...
This package implements a subset of R `relevent`. See [R effect coverage](docs/src/guide/r_catalogue.md) for validated effects and remaining limitations.

## Installation

Requires Julia 1.12+. This unreleased development version uses sibling source
paths. Clone its two dependencies beside it, then instantiate the package:
...
Start Julia with `julia --project` from Relevent.jl, or use the ecosystem root
workspace when all sibling packages are present.
...
- **Covariate effects**: static `CovSnd`, `CovRec`, `CovInt`, and directed `CovEvent`
- **Cumulative R effects**: normalized degrees, inverse recency ranks, three two-path/shared-partner effects, and actor fixed effects; see the [coverage table](docs/src/guide/r_catalogue.md)
...
- **Standardized entry point**: `fit_relevent` (alias `rem_dyad`), dispatching between the ordinal and interval likelihoods via `ordinal=true/false` with a shared maximum-likelihood interface
...
## Finite estimates and separation

Both fitters reject a proved separating direction with `ArgumentError` when no
finite MLE exists. A final Newton step or coefficient direction must pass exact
verification across the risk sets; the timing check includes all events and
positive exposure, including censored tails. Tiny genuine overlap is not rounded
to zero. The candidate search is not exhaustive, so a returned fit still needs
convergence and uncertainty checks. See the timing guide for the certificate
conditions; more iterations cannot fix a model with a proved infinite estimate.
...
# Interval timing requires effects constant between events
timing_stats = [PShift(:AB_BA), CovSnd(z)]
result = fit_relevent(events, timing_stats, n_actors; ordinal=false, t0=0.0)
...
Model event choice conditional on the observed history. This example uses
elapsed-time decay, so timestamps need meaningful units. If only order is known,
choose participation shifts or cumulative effects instead:
...
Fit an exponential timing model with statistics that remain constant between
events. Finite-half-life statistics are rejected: their changing hazard requires
exposure integration that this fitter does not implement. Use participation
shifts, cumulative R effects, fixed effects or static covariates. `halflife=Inf`
explicitly specifies zero decay; it changes the model. Finite decay remains
available for ordinal/conditional estimation.

```julia
# Interval-constant effects for timing
stats = [PShift(:AB_BA), CovSnd(collect(1.0:n_actors))]
...
## Fitted-result interface

Both fit types implement `coef`, `coefnames`, `stderror`, `vcov`, `confint`,
`loglikelihood`, `nobs`, `dof`, `aic`, `aicc`, `bic`, and `coeftable`. Timing
accessors include the log baseline first; `.coefficients` and `.std_errors`
remain effect-only for the hazard helpers. Results retain `iterations` and
`converged`. Non-convergence warns, and singular information gives NaN
uncertainty rather than finite pseudoinverse standard errors.

The ordinal and timing estimators use `Networks.newton_fit`. Full risk sets
are exact on untied data. `REM.fit_rem` can use sampled or full risk sets; its
sampling and covariance options have their own documented contracts.
